...
class JiraDaoTest {
  ...
  void createWorklog_newRecord() {
    ...
    Worklog workLogUtc = randomWorklog();
  ...
  void createWorklog_withExistingWorklog() {
    ...
    Worklog anotherWorkLogUtc = randomWorklog();
    ...
    Worklog expectedSavedWorkLog = convertWorkLogWithZone(anotherWorkLogUtc, perthTz);
  ...
  private Optional<Worklog> getWorklog(final long worklogId) {
    return fluentJdbc.query().select("SELECT issueid, author, timeworked, created, worklogbody FROM worklog WHERE id = ?")
        ...
        .firstResult(resultSet -> new Worklog()
            ...
            .setCreated(LocalDateTime.parse(resultSet.getString(4), DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss[x]")))
