...
public class JiraConnector implements WiseTimeConnector {
  ...
  private Map<String, Set<String>> projectClients;
  ...
  public void init(final ConnectorModule connectorModule) {
    ...
    projectClients = buildProjectClientsMap();
  }
    ...
    try {
      final List<UpsertTagRequest> upsertRequests = issues
          ...
          .map(issue -> {
            ...
            if (projectClients.containsKey(issue.getProjectKey())) {
              request.setExternalClientIds(List.copyOf(projectClients.get(issue.getProjectKey())));
            }
            return request;
          })
          .collect(Collectors.toList());
  ...
  Map<String, Set<String>> buildProjectClientsMap() {
    String projectClientJson = RuntimeConfig.getString(JiraConnectorConfigKey.PROJECT_CLIENTS_MAP)
        .map(env -> new String(Base64.decodeBase64(env), StandardCharsets.UTF_8))
        .orElse("{}");
    try {
      return new ObjectMapper().readValue(projectClientJson, new TypeReference<>() {
      });
    } catch (JsonProcessingException e) {
      log.warn("Failed to parse project-clients map", e);
      return Map.of();
    }
  }
